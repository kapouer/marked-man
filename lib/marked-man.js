...
inline.roff = merge({}, inline.normal, {
  mailto: /^mailto:/
});
...
function InlineLexer(links, options) {
  ...
  } else if (this.options.roff) {
    this.rules = inline.roff;
...
InlineLexer.prototype.roffOutput = function(src) {
  var out = ''
    , link
    , text
    , href
...
InlineLexer.prototype.roffOutputLink = function(cap, link) {
  var text = this.output(cap[1]);
  var href = resc(link.href);
  var same = text == href;
  if (same) return text;
  if (this.inline.mailto.test(link.href)) {
    return '\n.MT '
      + href
      + '\n'
      + text
...
Parser.prototype.parse = function(src) {
  ...
  if (this.options.format == "roff") {
    this.tok = this.roffTok;
    this.inline.output = this.inline.roffOutput;
    this.inline.outputLink = this.inline.roffOutputLink;
  }
...
Parser.prototype.roffTok = function() {
  switch (this.token.type) {
    case 'space': {
      return '';
    }
